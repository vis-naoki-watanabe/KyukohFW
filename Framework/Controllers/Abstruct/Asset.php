<?php
class Framework_Controllers_Abstruct_Asset extends Framework_Controllers_Abstruct
{
    ...
    {
        // 呼び出し元のクラスのパス取得
        $reflection_path = $this->getReflectionClassDir();
        $this->view->path = $reflection_path.'/../asset/'.$this->getRequest('path');
        if(!$this->isFile($this->view->path)) return;
    ...
    {
        if(!$filename = $this->isFile($this->view->path)) return;
